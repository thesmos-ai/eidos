...
// the rendered builder is trivially empty (every field filtered
// out upstream). A vacuous Builder would compile but defeat the
// compile check's purpose: the test exists to prove the bridge's
// scalar / well-known / optional / nested composition rules
// produced compilable Go, which can only happen when the
// builder's setters reference each translated type.
func assertBuilderExercisesBridge(t *testing.T, rendered []string) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(body, want) {
			t.Errorf("rendered builder missing %q\n--- rendered ---\n%s", want, body)
		}
	}
}
