...
// frontend/protobuf/testdata/<name> directory. Walks up from this
// test file to the workspace root (the directory carrying go.work)
// so the resolution is stable regardless of how many directory
// levels the test happens to live below the repo root.
func fixtureRoot(t *testing.T, name string) string {
	...
	return filepath.Join(dir, "frontend", "protobuf", "testdata", name)
}
