...
// WithBrand pins the tool identifier the backend stamps into
// the canonical "Code generated by <Brand>. DO NOT EDIT." header
// marker and the matching "<Brand>: end of generated content."
// footer line — also threaded into the manifest's Brand field.
// The prune subcommand consults the header marker before
// deleting any claimed file, so the Brand must match the binary
// identity the user invokes (a `testkit` binary stamps
// "testkit"; a `eidos-reference` binary stamps "eidos-reference").
//
// Empty (the default) falls back to the literal "eidos" — used
// by library / test callers that don't carry a meaningful brand.
