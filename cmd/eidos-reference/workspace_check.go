...
func preflightWorkspaceCheck(env *cli.Env) {
...
func emitWorkspaceNotice(env *cli.Env, modDir, workPath string) {
	...
	fmt.Fprintf(env.Stderr,
		"%s: notice: this directory's go.mod (%s) is not listed in the workspace at %s.\n"+
			...
			"  the golang frontend's options in .%s.yaml, OR add %q to the workspace's use() block.\n",
		brand, modDir, relWork, brand, relMod)
}
