...
// Command eidos is the reference binary for the eidos code-generation
// toolkit. It hardcodes the brand identifier "eidos", wires the Go
// frontend and backend, and registers every reference plugin shipped
// in this repository. Downstream consumers copy this main and
// substitute their own plugin slice.
//
// Subcommands:
//
//	eidos run     [flags] [patterns...]
//	eidos plan    [flags]
//	eidos explain [flags] <selector>
//	eidos check   [flags]
//	eidos prune   [flags]
//	eidos version [flags]
//
// Every subcommand accepts --config to point at a specific config
// file; absent that flag, the binary walks up from the working
// directory looking for `.eidos.yaml`.
package main
...
// brand is the hardcoded identifier this binary advertises to the
// cli package. Drives the rendered header / footer markers and the
// default `.eidos/` state directory.
const brand = "eidos"

// Subcommand names. Centralised so the help text, dispatch switch,
// and per-command FlagSet names stay aligned.
const (
	cmdRun     = "run"
	cmdPlan    = "plan"
	cmdExplain = "explain"
...
// Help-trigger tokens accepted in lieu of a subcommand: prints the
// top-level usage banner and exits ExitOK.
const (
	helpDashH    = "-h"
	helpDashDash = "--help"
	helpKeyword  = "help"
...
// usage is the top-level help string printed when no subcommand is
// supplied or the supplied subcommand is unknown.
const usage = `eidos - typed code generation toolkit

Usage:
  eidos <command> [flags] [args...]
...
Run "eidos <command> --help" for command-specific flags.
`

func main() {
	env, err := cli.NewEnv(brand)
	if err != nil {
		fmt.Fprintf(os.Stderr, "eidos: %v\n", err)
		os.Exit(cli.ExitUserError)
	}
	os.Exit(run(context.Background(), env, defaultPlugins(), os.Args[1:]))
}

// run dispatches argv (without the program name) to the appropriate
// command kernel. Extracted from main so tests can drive the
// argument-parsing surface against an in-memory env without
...
func run(ctx context.Context, env *cli.Env, plugins []plugin.Plugin, args []string) int {
	...
	default:
		fmt.Fprintf(env.Stderr, "eidos: unknown command %q\n\n%s", cmdName, usage)
		return cli.ExitUserError
	}
}

// defaultPlugins returns the static plugin universe this binary
// embeds: Go frontend + backend plus every reference plugin shipped
// in the repository. Downstream binaries replace this set with their
// own.
...
// loadConfigOrDefault resolves the config file: --config wins when
// non-empty, otherwise the binary walks up from Workdir looking for
// `.eidos.yaml`. A missing file is not an error — the defaults apply.
...
// --config flag. Returns the FlagSet plus a pointer to the parsed
// --config value.
...
// stderrOrStub returns env.Stderr or io.Discard when nil. Defensive
// against a partially-initialised env supplied by a downstream test
// harness — flag.FlagSet panics on a nil writer.
...
// dispatchRun parses run's flags, loads the config, and invokes the
// command. Positional args after the flags are the source patterns
...
func dispatchRun(ctx context.Context, env *cli.Env, plugins []plugin.Plugin, args []string) int {
	...
	if err != nil {
		fmt.Fprintf(env.Stderr, "eidos run: %v\n", err)
		return cli.ExitUserError
	}
	cmd.Config.File = cfg
...
func dispatchPlan(ctx context.Context, env *cli.Env, plugins []plugin.Plugin, args []string) int {
	...
	cfg, err := loadConfigOrDefault(env, *cfgPath)
	if err != nil {
		fmt.Fprintf(env.Stderr, "eidos plan: %v\n", err)
		return cli.ExitUserError
	}
	cmd.Config.File = cfg
	ensureDiag(env)
...
// dispatchExplain parses explain's flags, treats the first positional
// arg as the selector, and invokes the command.
func dispatchExplain(ctx context.Context, env *cli.Env, plugins []plugin.Plugin, args []string) int {
	...
	cfg, err := loadConfigOrDefault(env, *cfgPath)
	if err != nil {
		fmt.Fprintf(env.Stderr, "eidos explain: %v\n", err)
		return cli.ExitUserError
	}
	cmd.Config.File = cfg
	if fs.NArg() > 0 {
...
func dispatchCheck(ctx context.Context, env *cli.Env, plugins []plugin.Plugin, args []string) int {
	...
	cfg, err := loadConfigOrDefault(env, *cfgPath)
	if err != nil {
		fmt.Fprintf(env.Stderr, "eidos check: %v\n", err)
		return cli.ExitUserError
	}
	cmd.Config.File = cfg
	ensureDiag(env)
...
func dispatchPrune(ctx context.Context, env *cli.Env, plugins []plugin.Plugin, args []string) int {
	...
	cfg, err := loadConfigOrDefault(env, *cfgPath)
	if err != nil {
		fmt.Fprintf(env.Stderr, "eidos prune: %v\n", err)
		return cli.ExitUserError
	}
	cmd.Config.File = cfg
	ensureDiag(env)
...
func dispatchVersion(ctx context.Context, env *cli.Env, plugins []plugin.Plugin, args []string) int {
	...
	cfg, err := loadConfigOrDefault(env, *cfgPath)
	if err != nil {
		fmt.Fprintf(env.Stderr, "eidos version: %v\n", err)
		return cli.ExitUserError
	}
	cmd.Config.File = cfg
	ensureDiag(env)
...
// that construct an Env directly. NewEnv always populates the field;
// this guards against the bare-struct case.
